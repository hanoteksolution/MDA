...
class TenantListCreateView(APIView):
    ...
    def post(self, request):
        if not _can_tenants(request.user, "office_rental.tenants.create"):
...
class TenantDetailView(APIView):
    ...
    def get(self, request, pk):
        try:
            row = OfficeService.get_tenant(pk=pk, user=request.user, request=request)
    ...
    def _update(self, request, pk):
        if not _can_tenants(request.user, "office_rental.tenants.update"):
            return error_response(message="Forbidden.", status=status.HTTP_403_FORBIDDEN)
        try:
            row = OfficeService.get_tenant(pk=pk, user=request.user, request=request)
            row = OfficeService.update_tenant(
                office_tenant=row, data=request.data, user=request.user, request=request
            )
        except ObjectDoesNotExist:
            return error_response(message="Tenant not found.", status=status.HTTP_404_NOT_FOUND)
    ...
    def delete(self, request, pk):
        if not _can_tenants(request.user, "office_rental.tenants.delete"):
            return error_response(message="Forbidden.", status=status.HTTP_403_FORBIDDEN)
        try:
            row = OfficeService.get_tenant(pk=pk, user=request.user, request=request)
        except ObjectDoesNotExist:
            return error_response(message="Tenant not found.", status=status.HTTP_404_NOT_FOUND)
        OfficeService.soft_delete_tenant(
            office_tenant=row, user=request.user, request=request
        )
