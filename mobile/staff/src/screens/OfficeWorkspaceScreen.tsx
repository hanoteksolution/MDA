...
import { fetchOfficeSummary } from "@/api/bootstrap";
import { Card, Screen } from "@/components/Screen";
import type { RootStackParamList } from "@/navigation/types";
import { colors } from "@/theme/colors";

type Props = NativeStackScreenProps<RootStackParamList, "OfficeWorkspace">;
...
export function OfficeWorkspaceScreen({ navigation }: Props) {
  ...
  const reload = useCallback(async () => {
    setError("");
    try {
      setSummary(await fetchOfficeSummary());
    } catch (err) {
      setSummary(null);
      setError(err instanceof Error ? err.message : "Failed to load office summary");
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      <Card>
        <Text style={styles.kpiLabel}>Office vacant / occupied</Text>
